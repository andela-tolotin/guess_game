...
class InitializeGame
  ...
  def init
    ...
    cleaned_mode = obtain_and_clean_user_choice
    evaluate_player_instruction(cleaned_mode)  
  end

  def evaluate_player_instruction(cleaned_mode)
    mode_options = ["p","b","i","q"]
    if mode_options.include? cleaned_mode 
      case cleaned_mode
  ...
  def run_guesses(comp_guess,attempts=0,guesses=0)
    ...
    while guesses < 12 do 
      ...
      if player_guess.to_s == "q"
        ...
        if player_guess == "c"
           display_cheat_code # This line displays the cheat requested by the player
          ...
          if comp_guess.zip(p_guess).all? { |c,p| c===p } && guesses >= 12
           display_player_with_last_attempt_message(comp_guess,player_guess,attempts)
            break
          elsif comp_guess.zip(p_guess).all? { |c,p| c===p }
            display_player_message(comp_guess,player_guess,attempts) #display the player attempts
            break
          elsif guesses >= 12
            end_time = Time.now 
  ...
  def display_cheat_code
     ...
     Message.new("Enter the code below to win!")
  end

  def display_player_message(comp_guess,player_guess,attempts)
    end_time = Time.now
    Message.new(Computer.partial_and_matches(comp_guess,player_guess))
  ...
  def display_player_with_last_attempt_message(comp_guess,player_guess,attempts)
     Message.new(Computer.partial_and_matches(comp_guess,player_guess))
...
InitializeGame.new #Start playing the game here